...
def main(argv=None):
    ...
    for entry in wanted:
        ...
        if not src:
            print(f"  !! {entry['repo']} not found on the account, using local info fallback")
            src = {
                "name": entry["repo"],
                "description": entry.get("description") or "No description yet.",
                "language": entry.get("language") or "TypeScript",
                "stargazers_count": entry.get("stars") or 0,
                "forks_count": entry.get("forks") or 0,
            }
        card = {
            "name": src.get("name", entry["repo"]),
            "description": entry.get("description") or src.get("description"),
            "language": entry.get("language") or src.get("language"),
            "stars": src.get("stargazers_count", 0) if "stargazers_count" in src else src.get("stars", 0),
            "forks": src.get("forks_count", 0) if "forks_count" in src else src.get("forks", 0),
        }
        for theme in ("dark", "light"):
